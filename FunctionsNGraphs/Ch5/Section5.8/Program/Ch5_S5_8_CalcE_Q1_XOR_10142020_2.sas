...
%error3: %put ERROR: since an angle is supplied, you must provide one and only one of (side a, side b)!; %return;

%error4: %put ERROR: since "side a" is supplied, you must provide one and only one of  (side b, side c, angle a, or angle b)!; %return;

%error5: %put ERROR: "side a" must be less than "side c"; %return;

%error6: %put ERROR: since "side b" is supplied, you must provide one and only one of  (side a, side c, angle a, or angle b)!; %return;

%error7: %put ERROR: "side b" must be less than "side c"; %return;

%error8: %put ERROR: since "side c" is supplied, you must provide one and only one of (side a, side b)!; %return;

%error9: %put ERROR: if "angle a" or "angle b" is NOT supplied, please select and input EXACTlY 2 of ("side a", "side b", or "side c") 
without replacement; %return;

%code: %put THANK YOU: you have assigned arguments to macro parameters correclty -- well done!;
