...
scatter x=x y=g/name='sinx' legendlabel='y = sin(x)';
scatter x=x y=f/name='sinax' legendlabel='y = sin(abs(x)';
keylegend 'sinx' 'sinax' /title='Function Key' across=1 down=2;
refline 0/axis=y;
refline 0/axis=x;
...
panelby panel_c / columns = 2 novarname spacing = 10;
...
/*yaxis max=1 min=-1;*/
/*xaxis min=-6.3 max = 6.3 values = (-6.28 -4.71 -3.14 -1.57 0 1.57 3.14 4.71 6.28) valuesformat=pi2u_neg2pi_2pi_f.;*/
title 'q29: f(x) = sin(abs(x)) paneled';
run;
